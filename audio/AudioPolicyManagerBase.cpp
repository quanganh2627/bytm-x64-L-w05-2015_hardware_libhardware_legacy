    ...
    if (audio_is_output_device(device)) {
        ...
        if (device == AUDIO_DEVICE_OUT_WIRED_HEADSET) {
            ...
            device = AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET;
             ...
             ALOGD("setdeviceconnectionstate AUDIO_DEVICE_IN_DGTL_DOCK_HEADSET");
                                    ...
                                    const audio_offload_info_t *offloadInfo)
{
    ...
    if (IsRemoteBGMSupported(stream) && (flags != (AudioSystem::output_flags)AUDIO_OUTPUT_FLAG_REMOTE_BGM)) {
    ...
    if (audio_is_linear_pcm((audio_format_t)format)) {
        ...
        if (IsRemoteBGMSupported(stream) && (flags == 0)) {
           ...
           output = selectOutput(outputs, (AudioSystem::output_flags)tempflags);
        } else {
#endif
  ...
  // enabled music streams in addition to BGM sink stream
    if ((stream == AudioSystem::MUSIC)
        && ((flags == (AudioSystem::output_flags)AUDIO_OUTPUT_FLAG_REMOTE_BGM))
        && (mIsBGMEnabled)
...
void AudioPolicyManagerBase::doSetFmParameters(AudioParameter& param)
{
    ...
    if (param.get(key, strFmRoute) == NO_ERROR) {
        ...
        if (mFmIsOn != isFmOn) {
            mFmIsOn = isFmOn;
            LOGV("FM mode is on, %screment MUSIC stream count", mFmIsOn? "inc" : "dec");
                                                       ...
                                                       SortedVector<audio_io_handle_t>& outputs)
{
    ...
    if (state == AudioSystem::DEVICE_STATE_AVAILABLE) {
        ...
        for (ssize_t profile_index = 0; profile_index < (ssize_t)profiles.size(); profile_index++) {
            ...
            if (output != 0) {
                ...
                if (device == AUDIO_DEVICE_OUT_AUX_DIGITAL)
                {
                    ...
                    if (supportHDMI8)
                    {
                        ALOGV("DOLBY_ENDPOINT HDMI_8");
                        DolbySystemProperty::setHdmiCapability(CAP_HDMI_8);
                    }
                    else
                    {
                        ALOGV("DOLBY_ENDPOINT HDMI_6");
        ...
        if (device == AUDIO_DEVICE_OUT_AUX_DIGITAL)
        {
            ...
            ALOGV("DOLBY_ENDPOINT HDMI_INVALID");
        }
#endif //DOLBY_UDC
...
audio_devices_t AudioPolicyManagerBase::getNewDevice(audio_io_handle_t output, bool fromCache)
{
    ...
    //      use device for strategy media or fm_radio
